...
| 2026-07-30 | MGCN 全量审计复现 | GPU 连续生成两次 CSV | 排名与汇总稳定，但原始 CSV SHA-256 不同 | CUDA 稀疏运算存在无研究意义的末位浮点抖动 | 训练保留 GPU；正式审计推理切换 CPU。连续两次 CSV 哈希完全相同，空白前向差值为 0 | 已修正并复跑通过 |
...
| 2026-08-11 | Kimi 开发集 API | 80 条 v1 开发请求，每样本至多两次 | 12 条首次失败需重试；最终 5 条 `URLError`、1 条 share 约束失败 | 外部 HTTPS 连接波动；另有一次模型结构化数值不满足本地严格语义约束 | 完整保留 92 次尝试；6 条重试恢复，6 条永久失败按 ITT 计错，不做第三次请求或换样本 | 已完成并纳入开发分析 |
| 2026-08-11 | Kimi v2 开发集 API | 80 条强制选择请求，每样本至多两次 | 16 条发生重试；最终 5 条被原始日志误标为 `api_error` | 五条实际均为 `ResponseValidationError: explanation exceeds 1200 characters`；v2 异常类未绑定到通用运行器的捕获符号 | 不改原始日志；分析层纠正为 `schema_invalid`，后续 v2/确认集入口修正异常绑定；五条按 ITT 计错 | 已修正分类代码并分析 |
| 2026-08-11 | 确认集请求离线生成 | 首次扫描序列化请求中的禁用词 | 商品自然描述含普通单词 `contrast`，被误判为答案泄漏 | 检查器扫描了字段值而不是只扫描字段名 | 改为递归扫描 JSON key；590 条请求连续两次生成哈希一致，0 个答案字段 | 已修正并验收通过 |
| 2026-08-11 | Kimi v2 正式确认集 API | 590 条冻结请求，每样本最多两次 | 112 条发生重试；最终 40 条 schema 无效、3 条 API 失败 | 结构化解释超过本地约束或外部 API/网络失败 | 保留全部 702 次尝试；43 条永久失败按 ITT 计错，不做第三次请求、不换样本、不改提示词 | 已完成并纳入正式分析 |
