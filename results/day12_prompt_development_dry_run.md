...
在用户明确授权把公开商品图文及匿名化历史发送到 Kimi API 后，只运行了开发集第一
条，未继续发送剩余 79 条。固定配置：

- 模型：`kimi-k2.6`，请求与返回模型名一致；
- 图片：6 张；
- thinking：disabled；
- JSON Object mode；
- 最大输出：512 token；
- temperature / seed：提供方 API 不支持，明确记录而不伪造；
- 重试：0。
...
该开发样本的严格标签为 text，连续对比度为 `0.6581`。模型返回
`primary_evidence=both`、文字 share `0.6`、图片 share `0.4`、confidence `0.85`。
按预注册主指标，`both` 不算正确识别 text，因此这一条会记为方向错误。单个案例只
能验证管线，不能用于修改结论；它提示需要在完整 80 条开发集上检查模型是否系统性
偏爱 `both`，而不是立即针对这一题改提示词。

单样本响应日志 SHA-256：

`9CBDD2C8C874A79DD3A8EEA0CB2997001F6A20DF45BBC202D918AE641E6EAE9A`

后续状态：v1 的全部 80 条开发样本后来按每样本最多两次的规则完成；本节仅记录当时
的单样本验收快照，完整结果见 Day 13。
